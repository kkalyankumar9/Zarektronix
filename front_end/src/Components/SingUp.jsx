...
const SignUpPage = () => {
  ...
  const offlineSignupData = JSON.parse(
    localStorage.getItem("offlineSignup")
  ) || { name: "", email: "", password: "" };
  ...
  const handleSignup = async (e) => {
    ...
    try {
      if (isOnline) {
        if (name && email && password) {
          if (validatePassword(password)) {
            const response = await axios.post(
              "https://zarektronix-server.onrender.com/users/signup",
              {
                name,
                email,
                password,
        ...
        toast.info(
          "Signup data stored locally. It will be submitted when online."
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    const submitOfflineData = async () => {
      if (
        isOnline &&
        offlineSignupData &&
        validatePassword(offlineSignupData.password)
      ) {
        try {
          const response = await axios.post(
            "https://zarektronix-server.onrender.com/users/signup",
          ...
          console.error(
            "Error during offline signup submission:",
            error.message
          );
